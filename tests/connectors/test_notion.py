...
# ---------------------------------------------------------------------------
#
# query_database sets has_more/next_cursor from the response. Until it also
# assigned `_fetch_next`, anext_page() returned None while has_more was True, so
# a `while page:` loop or collect() stopped at page one and looked complete.

_DB_ID = "12345678-1234-1234-1234-123456789abc"


def _db_page(row_id: str, *, has_more: bool, next_cursor: str | None) -> dict:
    """One /databases/{id}/query response with a single row."""
    return {
        "object": "list",
        "results": [
            {
...
@pytest.mark.asyncio
async def test_query_database_anext_page_fetches_page_two(notion: Notion) -> None:
    """anext_page() must return page two, not None.

    Fails without the `_fetch_next` wiring: returns None despite has_more.
    """
    status_filter = {"property": "Status", "select": {"equals": "Done"}}
    sorts = [{"property": "Due", "direction": "ascending"}]

    with respx.mock(base_url="https://api.notion.com/v1") as respx_mock:
        route = respx_mock.post(f"/databases/{_DB_ID}/query").mock(
            side_effect=[
                httpx.Response(200, json=_db_page("row-1", has_more=True, next_cursor="CUR2")),
                httpx.Response(200, json=_db_page("row-2", has_more=False, next_cursor=None)),
            ]
        )

        page1 = await notion.aquery_database(
            database_id=_DB_ID, filter=status_filter, sorts=sorts, limit=1
        )
        ...
        assert [p.id for p in page2.items] == ["row-2"]
        assert page2.has_more is False
        assert await page2.anext_page() is None

        # The follow-up must send start_cursor AND keep filter/sorts — dropping
        # them would silently page through a different (unfiltered) row set.
        body = json.loads(route.calls[1].request.read())
        assert body["start_cursor"] == "CUR2"
        assert body["filter"] == status_filter
        assert body["sorts"] == sorts
        assert body["page_size"] == 1


@pytest.mark.asyncio
async def test_query_database_collect_returns_every_page(notion: Notion) -> None:
    """collect() is the agent-facing path; it must span both pages.

    Fails without the wiring: returns 1 row and looks complete.
    """
    with respx.mock(base_url="https://api.notion.com/v1") as respx_mock:
        respx_mock.post(f"/databases/{_DB_ID}/query").mock(
            side_effect=[
                httpx.Response(200, json=_db_page("row-1", has_more=True, next_cursor="CUR2")),
                httpx.Response(200, json=_db_page("row-2", has_more=False, next_cursor=None)),
            ]
        ...
        page1 = await notion.aquery_database(database_id=_DB_ID, limit=1)
        assert [p.id for p in await page1.collect()] == ["row-1", "row-2"]


@pytest.mark.asyncio
async def test_search_second_page_keeps_query_and_filter(notion: Notion) -> None:
    """search pages via start_cursor in the POST body and must keep its filters.
    ...
    with respx.mock(base_url="https://api.notion.com/v1") as respx_mock:
        route = respx_mock.post("/search").mock(
            side_effect=[
                httpx.Response(200, json=_db_page("pg-1", has_more=True, next_cursor="CUR2")),
                httpx.Response(200, json=_db_page("pg-2", has_more=False, next_cursor=None)),
            ]
        )

        page1 = await notion.asearch(query="roadmap", filter_type="page", limit=1)
        page2 = await page1.anext_page()

        assert page2 is not None
        assert [p.id for p in page2.items] == ["pg-2"]
        body = json.loads(route.calls[1].request.read())
        assert body["start_cursor"] == "CUR2"
