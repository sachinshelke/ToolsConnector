...
TOOL_META: dict[str, dict] = {
    ...
    "linkedin": {
        ...
        "overview": "The LinkedIn Marketing & Member APIs let you publish posts to a user's personal feed, comment on existing posts, and add reactions. Uses OAuth 2.0 with member-level scopes (w_member_social, r_member_social). DMs and mentions are NOT included — those endpoints require LinkedIn Partner Program approval (a contract, not OAuth scopes) and cannot be used with standard BYOK access.",
        "use_cases": ["Personal brand automation", "Thought leadership pipelines", "PR and announcement workflows", "Cross-posting from blogs", "Engagement automation"],
        "auth_methods": ["OAuth 2.0 Bearer Token"],
        "pricing": "API access free; tokens expire after 60 days",
        "rate_limit": "~100 calls/day per user (member-tier app)",
        "prerequisites": ["LinkedIn Developer App", "OAuth 2.0 access token with w_member_social, r_member_social, openid, profile, email scopes"],
        "get_credentials_url": "https://www.linkedin.com/developers/apps",
        "get_credentials_steps": "LinkedIn Developers > Apps > Create app > Auth > Generate token (60-day expiry)",
    },
    "mailchimp": {
    ...
    "x": {
        ...
        "rate_limit": "Tier-dependent; Free: ~50 writes/day; Basic: ~17K writes/month",
        "prerequisites": ["X Developer Account", "OAuth 2.0 user-context Bearer token", "Scopes: tweet.read tweet.write users.read offline.access (+ dm.read dm.write for DMs)"],
        "get_credentials_url": "https://developer.x.com/en/portal/dashboard",
        "get_credentials_steps": "X Developer Portal > Project > User authentication settings > OAuth 2.0 > Generate user-context access token",
    },
    "zendesk": {
