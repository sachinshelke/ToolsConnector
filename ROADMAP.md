...
- ⏳ **`gtasks`** — last connector in the Google Workspace batch. 13 actions. Pattern-correct + respx tests + Phase A applied + MCP tools/list verified via stdio JSON-RPC (13 tools exposed under `gtasks_*` prefix); live tools/call deferred until OAuth flow with `tasks` scope is run (this session's user token covered drive+docs+sheets+calendar but not `tasks`).
