...
- ⏳ **Google Workspace batch** — `gdrive`, `gsheets`, `gdocs`, `gtasks`, `gcalendar`. **Up next.** Promote all five from Tier 3 → Tier 1 in a single coordinated sweep. They share an auth model (Google OAuth 2.0 access tokens) and a discovery pattern (`googleapis.com/discovery/v1`), so the audit playbook can be applied once and reused across the batch with connector-specific live-verification scripts. Each gets: full doc cross-check against the live Discovery document, 5 rounds of respx tests, live verification with a Google account token (Drive: create folder + upload file + download; Sheets: create spreadsheet + write/read values + clear; Docs: create document + insert text + read; Tasks: create list + add task + complete + delete; Calendar: create event + update + delete + list across timezones), MCP end-to-end smoke, README expansion + ROADMAP promotion. Same shape as the Notion/Linear/GitHub sweeps already shipped.
