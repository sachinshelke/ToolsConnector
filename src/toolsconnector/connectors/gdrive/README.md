...
kit = GoogleDrive(credentials=auth)      # keeps working past the 1-hour expiry
```
...
missing = missing_scopes(creds, oauth_scopes(GoogleDrive))
if missing:
...
creds = asyncio.run(login_for(GoogleDrive, client_id="...",
    scopes=["https://www.googleapis.com/auth/drive.file"]))

# add Gmail later, keeping the existing grant
creds = asyncio.run(login_for(GoogleDrive, client_id="...",
    scopes=scopes_for(GoogleDrive, Gmail), incremental=True))
```

**Web apps:** use `begin_for(GoogleDrive, client_id=..., redirect_uri=...)` then
