...
kit = GoogleDocs(credentials=auth)      # keeps working past the 1-hour expiry
```
...
missing = missing_scopes(creds, oauth_scopes(GoogleDocs))
if missing:
...
creds = asyncio.run(login_for(GoogleDocs, client_id="...",
    scopes=["https://www.googleapis.com/auth/documents.readonly"]))

# add GoogleDrive later, keeping the existing grant
creds = asyncio.run(login_for(GoogleDocs, client_id="...",
    scopes=scopes_for(GoogleDocs, GoogleDrive), incremental=True))
```

**Web apps:** use `begin_for(GoogleDocs, client_id=..., redirect_uri=...)` then
