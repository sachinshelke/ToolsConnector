...
kit = GoogleTasks(credentials=auth)      # keeps working past the 1-hour expiry
```
...
missing = missing_scopes(creds, oauth_scopes(GoogleTasks))
if missing:
...
creds = asyncio.run(login_for(GoogleTasks, client_id="...",
    scopes=["https://www.googleapis.com/auth/tasks.readonly"]))

# add GoogleDrive later, keeping the existing grant
creds = asyncio.run(login_for(GoogleTasks, client_id="...",
    scopes=scopes_for(GoogleTasks, GoogleDrive), incremental=True))
```

**Web apps:** use `begin_for(GoogleTasks, client_id=..., redirect_uri=...)` then
